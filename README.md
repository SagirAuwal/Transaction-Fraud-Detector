---
title: Gojo Sentinel
emoji: 🛡️
colorFrom: green
colorTo: blue
sdk: docker
app_port: 7860
---

# Gojo Sentinel — AI Fraud Detection

This is a full-stack AI fraud detection system for Nigerian transactions.

## Features
- **Real-time Scoring**: Detects fraud in NIP, POS, USSD, and Web transactions.
- **Admin Dashboard**: Manage rules, users, and view history.
- **Mobile Ready**: Optimized for Android APK integration.

## Deployment
This space runs a FastAPI backend with a Vanilla JS frontend.
Built with XGBoost and Scikit-Learn.
